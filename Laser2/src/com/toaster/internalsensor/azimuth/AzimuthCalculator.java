...
public class AzimuthCalculator extends AbstractAzimuthCalculator implements SensorEventListener 
{
	//jujur gk ngerti mathnya... :(
	//http://stackoverflow.com/questions/16317599/android-compass-that-can-compensate-for-tilt-and-pitchs
	// raw inputs from Android sensors
	protected float m_Norm_Gravity;           // length of raw gravity vector received in onSensorChanged(...).  NB: should be about 10
    protected float[] m_NormGravityVector;    // Normalised gravity vector, (i.e. length of this vector is 1), which points straight up into space
    protected float m_Norm_MagField;          // length of raw magnetic field vector received in onSensorChanged(...). 
    protected float[] m_NormMagFieldValues;   // Normalised magnetic field vector, (i.e. length of this vector is 1)
    ...
    protected int m_GravityAccuracy;          // accuracy of gravity sensor
    protected int m_MagneticFieldAccuracy;    // accuracy of magnetic field sensor

    // values calculated once gravity and magnetic field vectors are available
    protected float[] m_NormEastVector;       // normalised cross product of raw gravity vector with magnetic field values, points east
    protected float[] m_NormNorthVector;      // Normalised vector pointing to magnetic north
    protected boolean m_OrientationOK;        // set true if m_azimuth_radians and m_pitch_radians have successfully been calculated following a call to onSensorChanged(...)
    protected float m_azimuth_radians;        // angle of the device from magnetic north
    protected float m_pitch_radians;          // tilt angle of the device from the horizontal.  m_pitch_radians = 0 if the device if flat, m_pitch_radians = Math.PI/2 means the device is upright.
    protected float m_pitch_axis_radians;     // angle which defines the axis for the rotation m_pitch_radians
    ...
    public AzimuthCalculator(InternalSensorHandler handler,int id)
    {
    	super(handler,id);
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (m_NormGravityVector != null && m_NormMagFieldValues != null) {
	        ...
	        else 
	        {
	            ...
	            handler.onAzimuthUpdated(deg360,this.id);
	        }
	    }		
	}
